...
export function PlaylistPreview({ playlist, handleSelect }) {
    ...
    return (
        <div className="playlist-preview-card shadow-lg p-3 mb-5 bg-body-tertiary rounded">
            <div className="playlist-preview-card-header">
                <h3 className="playlist-preview-card-name" title={playlist.name}>{playlist.name}</h3>
            </div>
            <div className="song-link" onClick={toggleSelected}>
                {selected ? (
                    <svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" fill="#70d6ff" className="bi bi-check-circle-fill" viewBox="0 0 16 16">
                        <path d="M16 8A8 8 0 1 1 0 8a8 8 0 0 1 16 0m-3.97-3.03a.75.75 0 0 0-1.08.022L7.477 9.417 5.384 7.323a.75.75 0 0 0-1.06 1.06L6.97 11.03a.75.75 0 0 0 1.079-.02l3.992-4.99a.75.75 0 0 0-.01-1.05z"/>
                    </svg>
                ) : (
                    <svg xmlns="http://www.w3.org/2000/svg" width="40" height="40" fill="black" className="bi bi-plus-circle-fill" viewBox="0 0 16 16">
                    <path d="M16 8A8 8 0 1 1 0 8a8 8 0 0 1 16 0M8.5 4.5a.5.5 0 0 0-1 0v3h-3a.5.5 0 0 0 0 1h3v3a.5.5 0 0 0 1 0v-3h3a.5.5 0 0 0 0-1h-3z"/>
                    </svg>
                )}
            </div>
            <div className="playlist-preview-card-date-container">
                <p className="playlist-preview-card-date">{playlist.date_created}</p>
            </div>
        </div>
    );
}
